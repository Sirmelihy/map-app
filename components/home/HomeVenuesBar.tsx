...
function CategoryDropdown({
    ...
    const itemClass = (active: boolean) =>
        `w-full flex items-center gap-2 px-3.5 py-2.5 text-xs transition-colors duration-100 cursor-pointer ${active
...
/** Collapsible bottom bar for the homepage — navigates to venue detail on click. */
export default function HomeVenuesBar({ venues }: Props) {
    ...
    const filtered = useMemo(
        () =>
            selectedCategory === "all"
                ? shuffled
                : shuffled.filter(
                    (v) => v.category.name === selectedCategory
                ),
        ...
        <Collapsible open={isOpen} onOpenChange={setIsOpen}>
            ...
            >
                ...
                <CollapsibleTrigger asChild>
                    <div className="flex items-center justify-between px-5 h-[44px] bg-slate-900/90 backdrop-blur-2xl border-t border-x border-white/10 rounded-t-2xl cursor-pointer select-none hover:bg-slate-800/90 transition-colors duration-200">
